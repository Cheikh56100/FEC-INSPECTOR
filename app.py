...
import fec_audit_pro_v5 as _fec_engine
import fec_patch_v6 as _patch
_patch.appliquer(_fec_engine)   # inject v2 functions + new benchmarks
...
from fec_audit_pro_v5 import (
    ...
    generer_commentaires, generer_rapport,
    generer_previsionnel, generer_rapport_mission_ia,
    ANTHROPIC_AVAILABLE, DOCX_AVAILABLE,
)
# Use patched versions
from fec_patch_v6 import (
    detecter_secteur_v2 as detecter_secteur,
    calculer_bouclier_fiscal_v2 as calculer_bouclier_fiscal,
    analyser_ecommerce,
    BENCHMARKS_SUPPLEMENTAIRES,
    SECTEURS_LISTE_COMPLET as SECTEURS_LISTE,
)
# Merged benchmarks (original + supplementaires)
BENCHMARKS = {**_fec_engine.BENCHMARKS}
...
def run_controls(df, secteur_code):
    ...
    controles = [
        ...
        ("Analyse E-commerce",  analyser_ecommerce,                             'ecommerce'),
    ]
    ...
    try:
        ...
        # Pass ecommerce stats so bouclier fiscal can use them
        if 'ecommerce' not in stats:
            stats['ecommerce'] = {}
...
# ── TAB 5 : E-commerce ───────────────────────────────────────
with tabs[4]:
    ec = stats.get('ecommerce', {})
    is_ecomm = secteur_code in ('ECOMMERCE_MARKETPLACE', 'ECOMMERCE_PROPRE')
    st.markdown('<div class="sec-header">🛒 Analyse E-commerce / Marketplace</div>',
                unsafe_allow_html=True)

    if not is_ecomm and not ec:
        st.info(
            "Ce module est optimisé pour les activités e-commerce (Amazon, Cdiscount…). "
            "Si votre entreprise vend en ligne, sélectionnez le secteur "
            "**E-commerce Marketplace** dans la barre latérale pour activer les contrôles spécifiques."
        )
    else:
        # KPIs e-commerce
        ca_ecomm = mg.get('ca', 0)
        ratio_top1 = ec.get('concentration_client_top1', 0)
        client_top1 = ec.get('client_top1', '—')
        frais_envoi = ec.get('frais_envoi', 0)
        ratio_envoi = ec.get('ratio_frais_envoi_ca', 0)
        commissions = ec.get('commissions_marketplace', 0)
        ratio_comm  = ec.get('ratio_comm_ca', 0)
        perso       = ec.get('charges_potentiellement_perso', 0)
        stock       = ec.get('solde_stocks', 0)

        col_e1, col_e2, col_e3, col_e4 = st.columns(4)
        c1_color = '#ef4444' if ratio_top1 > 90 else '#f97316' if ratio_top1 > 70 else '#22c55e'
        c2_color = '#ef4444' if ratio_envoi > 15 else '#f97316' if ratio_envoi > 8 else '#22c55e'
        c3_color = '#ef4444' if ratio_comm > 16 else '#f97316' if ratio_comm > 12 else '#22c55e'
        c4_color = '#f97316' if perso > 2000 else '#22c55e'

        kpi_ecomm = (
            '<div class="fec-kpi-grid">'
            + f'<div class="fec-kpi" style="border-color:{c1_color}">'
            + f'<div class="kpi-label">Concentration client</div>'
            + f'<div class="kpi-value" style="color:{c1_color}">{ratio_top1:.1f}%</div>'
            + f'<div class="kpi-sub">{client_top1[:20]}</div></div>'
            + f'<div class="fec-kpi" style="border-color:{c2_color}">'
            + f'<div class="kpi-label">Frais d\'envoi / CA</div>'
            + f'<div class="kpi-value" style="color:{c2_color}">{ratio_envoi:.1f}%</div>'
            + f'<div class="kpi-sub">{frais_envoi:,.0f} €</div></div>'
            + f'<div class="fec-kpi" style="border-color:{c3_color}">'
            + f'<div class="kpi-label">Commissions / CA</div>'
            + f'<div class="kpi-value" style="color:{c3_color}">{ratio_comm:.1f}%</div>'
            + f'<div class="kpi-sub">{commissions:,.0f} €</div></div>'
            + f'<div class="fec-kpi" style="border-color:{c4_color}">'
            + f'<div class="kpi-label">Charges mixtes pro/perso</div>'
            + f'<div class="kpi-value" style="color:{c4_color}">{perso:,.0f} €</div>'
            + f'<div class="kpi-sub">Stocks bilan : {stock:,.0f} €</div></div>'
            + '</div>'
        )
        st.markdown(kpi_ecomm, unsafe_allow_html=True)

        st.markdown("**Normes de référence marketplace (Amazon FBA/FBM)**")
        normes = [
            ("Concentration client unique", "< 60%", f"{ratio_top1:.1f}%",
             'pos' if ratio_top1 < 60 else 'neg'),
            ("Frais d'envoi / CA",          "< 8%",  f"{ratio_envoi:.1f}%",
             'pos' if ratio_envoi < 8 else 'warn' if ratio_envoi < 15 else 'neg'),
            ("Commissions marketplace / CA", "8–15%", f"{ratio_comm:.1f}%",
             'pos' if 8 <= ratio_comm <= 15 else 'warn' if ratio_comm < 18 else 'neg'),
            ("Stock / CA",                  "< 20%",
             f"{ec.get('solde_stocks',0)/mg.get('ca',1)*100:.1f}%" if mg.get('ca',0) else "—",
             'normal'),
            ("Charges mixtes / CA",         "< 1%",
             f"{perso/mg.get('ca',1)*100:.1f}%" if mg.get('ca',0) else "—",
             'pos' if perso/mg.get('ca',1) < 0.01 else 'neg' if mg.get('ca',0) else 'normal'),
        ]
        st.markdown("".join(row_html(l, f"{n}  →  {v}", k) for l, n, v, k in normes),
                    unsafe_allow_html=True)

        st.markdown("<br>**Anomalies spécifiques e-commerce**", unsafe_allow_html=True)
        ec_anom = [a for a in all_anomalies
                   if any(x in a.get('type','').lower() for x in
                          ['concentration', 'envoi', 'commission', 'stock',
                           'amazon', 'marketplace', 'mixte', 'personnel'])]
        if ec_anom:
            st.markdown("".join(anom_card(a) for a in ec_anom), unsafe_allow_html=True)
        else:
            st.markdown('<div class="fec-empty">✅ Aucune anomalie e-commerce détectée</div>',
                        unsafe_allow_html=True)


# ── TAB 6 : Prévisionnel N+1 ─────────────────────────────────
with tabs[5]:
